...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'sugar-cookie':
				dialogue = CoolUtil.coolTextFile(Paths.txt('sugar-cookie/pompomDialogue'));
			case 'key-lime':
				dialogue = CoolUtil.coolTextFile(Paths.txt('key-lime/keylimeDialogue'));
			case 'stomachache':
				dialogue = CoolUtil.coolTextFile(Paths.txt('stomachache/stomachacheDialogue'));
			case 'biscuit':
				dialogue = CoolUtil.coolTextFile(Paths.txt('biscuit/biscuitDialogue'));
			case 'gateau':
				dialogue = CoolUtil.coolTextFile(Paths.txt('gateau/gateauDialogue'));
			case 'pom-pomeranian':
				dialogue = CoolUtil.coolTextFile(Paths.txt('pom-pomeranian/pompomeranianDialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'sugar-cookie' | 'key-lime' | 'stomachache':
			{
				curStage = 'sweetstage';

				defaultCamZoom = 0.90;

				var sweetstagebg:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('pompom/pompombg'));
				...
				sweetstagecity.frames = Paths.getSparrowAtlas('pompom/pompomcity');
				...
				var sweetstagebgtrees1:FlxSprite = new FlxSprite(-600, 300).loadGraphic(Paths.image('pompom/pompombgtrees1'));
				...
				var sweetstagetreemid:FlxSprite = new FlxSprite(550, 150).loadGraphic(Paths.image('pompom/pompomtreemid'));
				...
				sweetstagehouse.frames = Paths.getSparrowAtlas('pompom/pompomhouse');
				...
				var sweetstagefg:FlxSprite = new FlxSprite(-500, 0).loadGraphic(Paths.image('pompom/pompomfg'));
				sweetstagefg.antialiasing = true;
				sweetstagefg.active = false;
				add(sweetstagefg);

				var sweetstagefgtree2:FlxSprite = new FlxSprite(1300, -10).loadGraphic(Paths.image('pompom/pompomfgtree2'));
				...
				var sweetstagetreefg:FlxSprite = new FlxSprite(500, -200).loadGraphic(Paths.image('pompom/pompomtreefg'));
				...
				sweetstagemackie.frames = Paths.getSparrowAtlas('pompom/mackiebob');
			...
			case 'biscuit':
			{
				...
				var mackiebg:FlxSprite = new FlxSprite(-2150, -1000).loadGraphic(Paths.image('mackie/citybg'));
				...
				var sewer:FlxSprite = new FlxSprite(1400, 722).loadGraphic(Paths.image('mackie/sewer'));
			...
			case 'gateau':
			{
				...
				smallbgcrowd.frames = Paths.getSparrowAtlas('mackie/smallcrowdbob');
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'pom-pomeranian':
					...
					dad.alpha = 0;
					var blackScreen:FlxSprite = new FlxSprite(-1000, -1000).makeGraphic(Std.int(FlxG.width * 6), Std.int(FlxG.height * 6), FlxColor.BLACK);
					//add(blackScreen);
					blackScreen.scrollFactor.set();
					camHUD.visible = false;
					camFollow.x = dad.getGraphicMidpoint().x-230;
					camFollow.y = dad.getGraphicMidpoint().y-200;
					...
					new FlxTimer().start(0.6, function(tmr:FlxTimer)
					{
						...
						FlxG.sound.playMusic(Paths.music('crowd'), 1, false);
						FlxG.camera.zoom = 1.1;
						new FlxTimer().start(1.4, function(tmr:FlxTimer)
						{
							dad.alpha = 1;
							dad.playAnim('pomintro');
							new FlxTimer().start(2.2, function(tmr:FlxTimer)
							{
								camHUD.visible = true;
								add(blackScreen);
									camFollow.x = gf.getGraphicMidpoint().x+150;
				...
				case 'biscuit':
					...
					blackScreen.scrollFactor.set();
					camHUD.visible = false;
					camFollow.x = dad.getGraphicMidpoint().x+600;
					...
					{
						//remove(blackScreen);
						camFollow.x = dad.getGraphicMidpoint().x+60;
						camFollow.y = dad.getGraphicMidpoint().y-50;
						...
						FlxG.camera.zoom = 1.5;
						FlxG.sound.playMusic(Paths.music('bark'), 1, false);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (curSong == "Stomachache"){
						FlxG.switchState(new EndingState(health >= 1));

					}if (curSong == "Pom-Pomeranian"){
						FlxG.switchState(new MackieEndingState(health >= 1));
	...
	*/
	...
	override function beatHit()
	{
		...
		//This is literally the worst code but im lazy and it works!!!!
